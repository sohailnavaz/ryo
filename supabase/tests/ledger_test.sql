...
begin
  perform pg_temp.act_as('11110000-0000-4000-8000-000000000a01');
  res := admin_action('booking.refund','booking','11110000-0000-4000-8000-0000000000c1',
                      'HOST_CANCELLED','plumbing', jsonb_build_object('amount_cents',100000), null, false);
  perform pg_temp.as_pg();
  if res->>'status' <> 'applied' then raise exception 'FAIL 7: refund status %', res->>'status'; end if;
...
begin
  perform pg_temp.act_as('11110000-0000-4000-8000-000000000a01');
  begin
    perform admin_action('booking.refund','booking','11110000-0000-4000-8000-0000000000c1',
                        'HOST_CANCELLED','too much', jsonb_build_object('amount_cents',99999999), null, false);
    raise exception 'FAIL 8: over-refund accepted';
  exception when sqlstate '22023' then
    get stacked diagnostics err = message_text;
    if err not like '%INVALID_PAYLOAD%' then raise exception 'FAIL 8: wrong error %', err; end if;
  end;
  perform pg_temp.as_pg();
