...
- **Professional / launch-readiness layer (2026-06-28).** Six cross-cutting additions landed, typecheck + `next build` green:
  - **Worldwide destination search.** Curated in-bundle gazetteer — **195 countries + 861 major cities** (`packages/api/src/places-data.ts`) — backs an always-available typeahead, with an exhaustive live tier in front of it: migration `0015_places.sql` (a `places` table + `pg_trgm` `search_places` RPC) seeded by `scripts/import-places.mjs` from GeoNames (`pnpm db:places`). `FilterSheet` now debounce-queries `usePlacesSearch` (DB-first, curated fallback).
  - **i18n scaffold + 7 languages.** EN/ES/FR/DE/AR(RTL)/ZH/JA catalogues (`packages/features/src/i18n/`), persisted locale store, `useT()` hook with English fallback, a language switcher (footer + account menu), and `<html lang/dir>` reflection for right-to-left Arabic.
  - **Legal section.** Privacy, Terms, Cookie, and Security & Trust pages under `/legal/*` (+ `/.well-known/security.txt`), each with canonical metadata.
  - **SEO upgrade.** Per-route `metadata`/`generateMetadata` (incl. dynamic listing titles + `LodgingBusiness` JSON-LD), site-wide Organization/WebSite JSON-LD with SearchAction, OpenGraph/Twitter cards, a dynamic `/opengraph-image`, and an expanded sitemap/robots.
  - **Guidance.** A searchable Help center / FAQ (`/faq`, 58 answers across 8 categories) + a first-run onboarding flow.
  - **Cookie consent + trust footer.** Granular consent banner (necessary/functional/analytics, reopenable) + a global footer with legal/support links and the language switcher.
